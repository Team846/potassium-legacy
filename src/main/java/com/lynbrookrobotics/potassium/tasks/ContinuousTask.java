 ...
 *
 * Example use: A task that spins the rollers on your collector. This would not have an end, but
 * could be mapped to a button to run while the button is held down.
 */
public abstract class ContinuousTask extends Task {
  ...
  /**
   * What to do at the end of the task, called from abort() Should include things such as resetting
   * component controllers and internal state The task should be in the same state as it was before
   * it was run after calling this method
   */
